...
class ReportConsistencyTests(unittest.IsolatedAsyncioTestCase):
    async def test_summary_trends_group_alignment_and_exports(self):
        report_service = load_module()
        summary = {
            "orders": {"total": 2, "created": 1, "delivered": 1, "picked_up": 0},
            "alerts": {"total": 1, "open": 1, "acknowledged": 0, "resolved": 0, "false_positive": 0},
            "devices": {"total": 1, "online": 1, "offline": 0},
            "sessions": {"total": 2, "armed": 1, "alerted": 1, "confirmed": 0},
            "events_last_24h": 4,
        ...
        trends = {
            ...
            "alerts": [{"day": "2026-03-15", "count": 1}],
            "devices": [{"day": "2026-03-15", "count": 1}],
            "sessions": [{"day": "2026-03-15", "count": 2}],
            "events": [{"day": "2026-03-15", "count": 4}],
            "rule_matches": [{"day": "2026-03-15", "count": 3}],
        }
        ...
        summary_groups = {key for key, value in summary.items() if isinstance(value, dict)}
        trend_groups = {key for key in trends.keys() if key != "interval"}
        expected_trend_groups = summary_groups | {"events"}
        self.assertEqual(trend_groups, expected_trend_groups)

        text_summary = summary_csv.decode("utf-8")
        text_trends = trends_csv.decode("utf-8")
        self.assertIn("devices", text_summary)
        self.assertIn("sessions", text_summary)
        self.assertIn("rule_matches", text_summary)
        self.assertIn("devices", text_trends)
        self.assertIn("sessions", text_trends)
