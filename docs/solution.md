# 外卖围栏取餐准入与防盗留痕 技术方案与里程碑

## 当前推荐落地方案

针对学校用餐高峰期外卖堆满、学生难以确认是否拿到自己外卖的问题，本项目当前主方案调整为“外卖点围栏 + 入口人员核验 + 一次性取餐码 + 留痕追溯”。

核心流程：

1. 外卖统一放入围栏内取餐点。
2. 学生在网页端或微信小程序 web-view 打开 `/pickup`，为自己的订单生成一次性取餐码。
3. 入口工作人员使用 `/gate` 页面扫码或输入取餐码。
4. 系统校验取餐码是否存在、过期、已使用。
5. 校验通过后记录 `gate_entry` 核验记录，工作人员放行学生进入围栏取餐区。
6. 摄像头、运动检测、告警与证据包能力保留为辅助取证和争议追溯，不再作为核心准入判断手段。

该方案避免上智能柜、每格称重和人脸识别，把成本集中在入口管理和软件留痕上，更适合校园试点。

## 网页端与微信小程序访问

- 网页端：直接访问 PWA 的 `/gate` 和 `/pickup`。
- 微信小程序：使用小程序 `web-view` 承载同一 H5 页面。
- 生产环境要求：HTTPS 域名、ICP备案、微信小程序后台配置业务域名。
